...
public class LightLocalizer {
   ...
   private float d = 10.8f; //light sensor distance
	...
	public void doLocalization() {
      ...
      float thetaY = Math.abs(theta[0]-theta[2]);//theta over y-axis (FOR X CORRECTION)
      float thetaX = Math.abs(theta[1]-theta[3]);//theta over x-axis (FOR Y CORRECTION)
      float thetaC = 270-theta[0]+theta[2]/2;//error in theta
      //apply fomula
      position[0] = -d*(float)Math.sin(thetaY/2);//x computed
      position[1] = -d*(float)Math.cos(thetaX/2);//y computed
